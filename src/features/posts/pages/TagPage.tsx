...
export function TagPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-24 pt-10">
        ...
        {posts.map((p) => (
          <li key={p.slug} className="py-5">
            <Link to={postPath(p.slug)} className="group block">
              <span className="font-semibold text-slate-950 group-hover:text-gold-ink dark:text-slate-100 dark:group-hover:text-gold">
                {p.title}
              </span>
              <span className="mt-1 block max-w-[65ch] text-sm text-slate-600 dark:text-slate-400">
                {p.description}
              </span>
      ...
      {posts.length === 0 && (
        <p className="text-slate-600 dark:text-slate-400">Nothing here yet.</p>
      )}
      <Link
        to={routes.home}
        className="mt-8 inline-block text-sm text-gold-ink hover:underline dark:text-gold"
      >
        ← All posts
      </Link>
    </div>
